...
COIN_NUMS = 7                                      # 분산 투자 코인 개수 (자산/COIN_NUMS를 각 코인에 투자)
LARRY_K = 0.5

GAIN = 0.03                                          # 30% 이상 이익시 50% 물량 익절
DUAL_NOISE_LIMIT1 = 0.65                            # 듀얼 노이즈가 0.75 이하인 종목만 투자
PROFIT_CUT = 0.03 
...
def try_buy(tickers, prices, targets, multiple_mas, budget_per_coin, holdings, profit_cut):
    ...
    try:
        for ticker in tickers:
            ...
            if price > target and (price / target) < 1.004 and holdings[ticker] is False and profit_cut[ticker] is False:
